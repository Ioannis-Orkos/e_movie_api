...
if(!empty($user_g_id)&&!empty($store_id)&&!empty($movie_id) ){
    ...
    // query products
    $stmt = $store->store_mangRequestMovieRent($movie_id,$user_g_id,$store_id,$req_status,$store_msg);
    $num = $stmt->rowCount();
    ...
    if($num>0){
        ...
        echo json_encode(array("message" => "{$req_status}"));
         $store->store_mangRequestMovieRentNotification($movie_id,$user_g_id,$store_id,$req_status);
