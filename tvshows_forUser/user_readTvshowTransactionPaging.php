...
$user_g_id = isset($_GET["ugid"])?$_GET["ugid"]:"";
...
// query products
$stmt = $user->user_readTvshowTransactionPaging($from_record_num, $records_per_page,$user_g_id,$status);
$num = $stmt->rowCount();
...
// var_dump($row);
        ...
        $result_data["tvshows_result"]=array(
          "imdb_id"  => $imdb_id,
          "title" => $title,
          "year" =>  $year,
          "released" =>   $released,
    ...
    // include paging
    $total_rows=$user->user_readTvshowTransactionCount($user_g_id,$status);
    $page_url="{$home_url}users/user_readTvshowTransactionPaging.php?";
