...
// query products
$stmt =  $tvshows->readPaging_searchTvshows_forUser($from_record_num, $records_per_page,$search,$mType,$mGenre,$mSort,$mYear,$mOrder);
$num  =  $stmt->rowCount();
...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        array_push($tvshows_arr["records"], $movie_one);
    }


    // include paging
    $total_rows= $tvshows->readPagingcount_searchTvshows_forUser($search,$mType,$mGenre,$mSort,$mYear,$mOrder);
    $page_url="{$home_url}tvshows_forUser/tvshow_search_readByPaging.php";
